...
@Component({
  selector: 'app-transaction-ncmc-list',
  templateUrl: './transaction-ncmc-list.component.html',
  styleUrls: ['./transaction-ncmc-list.component.scss']
})
export class TransactionNcmcListComponent implements OnInit {
  ...
  getFilteredResponse(){
    ...
    this.adminSrv.customFilterPostRequestForNCMC(reqObj).subscribe({
      next:(resp:any)=>{
        if(resp["status"] === "1"){
          this.txnList = [];
          this.temp  = true;
  ...
  onFileExtensionChange(e:any){
    ...
    this.adminSrv.downloadNcmcTxnFile(reqObj, fileExt).subscribe({
      next:(resp:any)=>{
        const blob = new Blob([resp], {type:'*/*'});
        saveAs(blob,`NcmcTransaction.${fileExt}`);
